...
function editEmployee(id){const e=data.employees.find(x=>x.id===id);if(!e)return;$('employeeId').value=e.id;Object.keys(e).forEach(k=>{if($(k))$(k).value=e[k]??''});$('employeeFormTitle').textContent='社員情報を編集';showPage('employee-form')}
...
function leavePeriod(month){
  ...
  if(value===3)return{start:`${year}-02-16`,end:`${year}-03-31`};
  if(value===4)return{start:`${year}-04-01`,end:`${year}-04-15`};
  const previous=new Date(year,value-2,1);
